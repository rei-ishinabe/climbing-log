class Gym < ApplicationRecord
  ...
  def last_visit_date(user)
    routes.where(user_id: user.id).joins(:logs).order(date: 'DESC').first.logs.order(date: 'DESC').first.date.to_date if routes.where(user_id: user.id).joins(:logs).exists?
  end
end
